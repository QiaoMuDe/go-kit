package utils // import "gitee.com/MM-Q/go-kit/utils"
...
func ExecuteCmd(args []string, env []string) ([]byte, error)
    ...
    参数:
      - args: 命令行参数切片，其中 args[0] 为要执行的命令本身（如 "ls", "go"）， 后续元素为命令的参数（如 "-l",
        "main.go"）。
      - env: 一个完整的环境变量切片，形如 "KEY=VALUE"。 如果传入 nil 或空切片，则命令将继承当前进程的环境变量。
        如果传入非空切片，则命令的环境变量将仅限于此切片中定义的内容， 不会继承当前进程的任何环境变量。
