...
func layout(g *gocui.Gui) error {
	...
	if v, err := g.SetView("main", 79, 0, maxX, maxY-1); err != nil {
		...
		_, _ = fmt.Fprintf(v, "%s", filenamesBytes)
		v.Editable = false
		v.Wrap = true
		v.Frame = true
...
func finder(v *gocui.View, key gocui.Key, ch rune, mod gocui.Modifier) {
	...
	case ch != 0 && mod == 0:
		...
		g.Update(func(gui *gocui.Gui) error {
			...
			_, _ = fmt.Fprintf(results, "found %v matches in %v\n", len(matches), elapsed)
			for _, match := range matches {
				for i := 0; i < len(match.Str); i++ {
					if contains(i, match.MatchedIndexes) {
						_, _ = fmt.Fprintf(results, "\033[1m%s\033[0m", string(match.Str[i]))
					} else {
						_, _ = fmt.Fprintf(results, "%s", string(match.Str[i]))
					}

				}
				_, _ = fmt.Fprintln(results, "")
			}
			return nil
		})
